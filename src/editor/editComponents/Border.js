...
const Border = ({ updateProperties, selectedComponent, parsePixelValueToNumber }) => {
    // Border
    const [border, setBorder] = useState({
        borderTopWidth: parsePixelValueToNumber(selectedComponent?.style?.borderTopWidth),
        borderBottomWidth: parsePixelValueToNumber(selectedComponent?.style?.borderBottomWidth),
        borderLeftWidth: parsePixelValueToNumber(selectedComponent?.style?.borderLeftWidth),
        borderRightWidth: parsePixelValueToNumber(selectedComponent?.style?.borderRightWidth),
    })

    const handleBorderChange = (value, type) => {
        let updatedStyle = { ...selectedComponent.style }

        // Handle border types
        switch (type) {
            case 'borderTopWidth':
                updatedStyle = { ...updatedStyle, borderTopWidth: `${value}px` }
                setBorder({ ...border, borderTopWidth: value })
                break
            case 'borderBottomWidth':
                updatedStyle = { ...updatedStyle, borderBottomWidth: `${value}px` }
                setBorder({ ...border, borderBottomWidth: value })
                break
            case 'borderLeftWidth':
                updatedStyle = { ...updatedStyle, borderLeftWidth: `${value}px` }
                setBorder({ ...border, borderLeftWidth: value })
                break
            case 'borderRightWidth':
                updatedStyle = { ...updatedStyle, borderRightWidth: `${value}px` }
                setBorder({ ...border, borderRightWidth: value })
                break
            default:
    ...
    return (
        <div className='flex justify-between items-center'>
            <label className='text-xs text-subItemGray font-medium block  shrink-0'>Border</label>
                ...
                <div className='flex justify-between gap-2'>
                    <div className='flex items-center bg-[#F2F2F2] rounded p-2 w-[64px] h-[32px]'>
                        <input
                            ...
                            value={border.borderTopWidth}
                            onChange={(e) => handleBorderChange(e.target.value, 'borderTopWidth')}
                        />
                        <svg
                            width='8'
                            height='9'
                    ...
                    <div className='flex items-center bg-[#F2F2F2] rounded p-2 w-[64px] h-[32px]'>
                        <input
                            type='number'
                            className='w-full rounded text-xs font-semibold bg-[#F2F2F2] text-almostBlack  outline-none'
                            value={border.borderBottomWidth}
                            onChange={(e) => handleBorderChange(e.target.value, 'borderBottomWidth')}
                        />
                        <svg
                            width='7'
                            height='9'
                ...
                <div className='flex justify-between gap-2 mt-2'>
                    <div className='flex items-center bg-[#F2F2F2] rounded p-2 w-[64px] h-[32px]'>
                        <input
                            type='number'
                            className='w-full rounded text-xs font-semibold bg-[#F2F2F2] text-almostBlack  outline-none'
                            value={border.borderLeftWidth}
                            onChange={(e) => handleBorderChange(e.target.value, 'borderLeftWidth')}
                        />
                        <svg
                            width='6'
                            height='9'
                    ...
                    <div className='flex items-center bg-[#F2F2F2] rounded p-2 w-[64px] h-[32px]'>
                        <input
                            type='number'
                            className='w-full rounded text-xs font-semibold bg-[#F2F2F2] text-almostBlack  outline-none'
                            value={border.borderRightWidth}
                            onChange={(e) => handleBorderChange(e.target.value, 'borderRightWidth')}
                        />
                        <svg
                            width='7'
                            height='9'
